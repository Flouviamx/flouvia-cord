...
14. **Nunca nombres de variables de entorno ni de proveedores de infraestructura en texto que ve el cliente (Ago 2026):** el negocio que usa Cord no sabe ni le importa qué corre por debajo — `RESEND_API_KEY`, `FACTURAPI_USER_KEY`, "Vercel", "Facturapi" son vocabulario NUESTRO, no suyo. Un mensaje de error o un panel de estado que dice *"Falta FACTURAPI_USER_KEY en el entorno — agrégala en Vercel"* es un error interno filtrado a producción: no es accionable para quien lo lee y expone infraestructura que un atacante no necesitaba conocer. La misma clase de bug apareció **tres veces en una sola sesión** (`agentes.astro`/`correo.astro` mencionando `RESEND_API_KEY`, `/api/fiscal/csd.ts` mencionando `FACTURAPI_USER_KEY`) y una **cuarta vez como panel completo** (Ajustes › Fiscal mostraba "Facturapi · CFDI 4.0" y una etiqueta técnica "Cuenta de prueba" — el dueño del negocio no necesita saber CON QUÉ CUENTA se timbra, solo si su propio CSD está conectado). Regla: cualquier mensaje/panel dirigido al dueño del negocio describe el **estado** ("esto no está disponible todavía, escríbenos"), nunca el **mecanismo** (qué proveedor, qué variable, qué archivo de config). Nombrar el proveedor real sigue siendo correcto en `docs.cordhq.app` (documentación técnica para desarrolladores que si necesitan saberlo) — la regla aplica a la UI de la app y a los mensajes de error de la API que esa UI muestra.

15. **Una preferencia que se guarda pero que ningún emisor consulta es una promesa falsa (Ago 2026):** antes de esta regla, Cord acumuló **tres** superficies así — `orgs.notif_prefs` (matriz de Ajustes › Notificaciones) se guardaba desde jun 2026 pero **ningún correo/Slack la consultaba jamás** (Slack posteaba TODOS los eventos sin mirarla); `orgs.integraciones` (toggles de Shopify/Woo/Meli/Zapier/CONTPAQi en Ajustes › Integraciones) se guardaba pero no conectaba nada — un toggle "activado" con badge "Pendiente de conexión" seguía sin funcionar jamás; y la llave de MCP en el snippet de conexión era literalmente `sk_live_xxxxxxxxxxxx`, un placeholder con forma de llave real que nunca funcionaría. Cada una se sintió "terminada" (la UI guardaba, mostraba un check verde) sin que el usuario tuviera forma de saber que no hacía nada. **Regla: al agregar un toggle/preferencia/dato nuevo, el consumidor real (quién LEE ese valor y actúa sobre él) se cablea en el MISMO cambio — no después.** Si construir el consumidor real no es viable todavía, la UI debe decir explícitamente "Próximamente" (sin nombrar conectores específicos que no existen — ver Regla 14 sobre no prometer de más) en vez de ofrecer un control interactivo que parece funcionar. Verificar esto es barato: `grep` del nombre de columna/preferencia fuera del archivo que la escribe — si el único resultado es la propia página que la renderiza, es una promesa falsa.

16. **Móvil es táctil, no escritorio encogido (Regla Estricta - Ago 2026):** bajo 880 px ninguna función esencial puede depender de `hover`, drag preciso o un atajo de teclado. Los indicadores `<kbd>`, la ayuda de atajos y los listeners de comandos globales se ocultan o desactivan solo en ese breakpoint; escribir en campos, usar el teclado virtual y confirmar formularios con `Enter` sigue permitido. Los widgets se apilan en una columna y se editan con botones visibles de subir, bajar y ocultar de al menos 44 px; arrastrar y redimensionar siguen disponibles únicamente en escritorio. Toda gráfica interactiva debe responder a toque, mantener el detalle visible después del tap y poder cerrarlo tocando fuera. Esto no autoriza a ocultar importes, estados, fechas, cantidades ni ninguna otra información comercial, y no debe alterar el layout ni los atajos de escritorio.
