...
| Microsoft Teams con "Conectar con Microsoft" (inactivo sin `TEAMS_CLIENT_*`) | Sección "Canales de aviso" de [`docs/estado/integraciones.md`](docs/estado/integraciones.md) + `src/lib/integraciones/teams-graph.ts`; `deliverTeams()` es la única salida para avisos, workflows y prueba |
...
| **Pendiente de código en Mercado Pago:** (1) cobrar facturas con Mercado Pago, (2) que Cord lea los reembolsos hechos en Mercado Pago, (3) link de pago en la cobranza con IA cuando el único riel es Mercado Pago | Sección Mercado Pago de [`docs/estado/pendientes-integraciones.md`](docs/estado/pendientes-integraciones.md). Dónde se hace: (1) `src/pages/i/[token].astro` + `src/pages/api/i/[token]/` siguiendo a `src/pages/api/q/[token]/mp-preference.ts`; (2) `src/pages/api/mercadopago/webhook.ts`, que hoy solo actúa con `approved`; (3) `cobra_online` en `src/lib/agents/cobranza-run.ts`, que solo mira Stripe. Toda ruta nueva de dinero cumple la regla 33 |
