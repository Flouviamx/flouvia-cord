...
// Mismo motor de gradiente que SupportCoverBg.jsx (FBM domain-warp, Reinhard
// tonemap + dither) pero con una paleta de azul más clara/vívida para el hero
// del Roadmap — mismo tratamiento "hero oscuro", distinto tono.
...
void main() {
  ...
  float glow = exp(-length(uv - vec2(0.5, 0.4)) * 3.5) * 0.08;
  col += u_cc * glow;

  // Fade oscuro hacia arriba (bajo el navbar, mismo patrón que el hero de soporte)
  float topFade = smoothstep(0.0, 0.55, uv.y);
  col *= mix(0.58, 1.0, topFade);
...
// Azul más claro/vívido que el de soporte — navy medio → azul → azure brillante.
const PALETTE = [[0.03, 0.09, 0.22], [0.08, 0.24, 0.52], [0.24, 0.52, 0.94]];
...
export default function RoadmapCoverBg() {
