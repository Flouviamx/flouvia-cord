...
export default function CustomUserProfile({ locale = 'es', user: initialUser }: { locale?: Locale; user?: UserProfile | null }) {
  ...
  // Conectar = navegación completa al proveedor (no fetch): el flujo OAuth
  // necesita el roundtrip del navegador. Vuelve a esta misma página con
  // ?linked= o ?link_error=, que se leen en el efecto de abajo.
  ...
  useEffect(() => {
    ...
    if (linked) {
      toast(locale === 'en' ? 'Account connected' : 'Cuenta conectada');
    } else {
      const provider = params.get('provider') === 'apple' ? 'Apple' : 'Google';
      const messages: Record<string, string> = {
        already_linked: locale === 'en'
          ? `That ${provider} account is already linked to another Cord account.`
          : `Esa cuenta de ${provider} ya está vinculada a otra cuenta de Cord.`,
        unavailable: locale === 'en'
          ? `${provider} sign-in is not configured yet.`
          : `El acceso con ${provider} todavía no está configurado.`,
    ...
    params.delete('linked'); params.delete('link_error'); params.delete('provider');
