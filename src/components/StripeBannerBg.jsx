...
const fragmentShader = /* glsl */`
  ...
  void main() {
    ...
    // ── Paleta Cord (Quiet Luxury): Azul Navy, Azul Eléctrico y Esmeralda ──
    vec3 colorNavy    = vec3(0.04, 0.10, 0.18); // #0a192f (Cord Deep Blue)
    vec3 colorBlue    = vec3(0.15, 0.39, 0.92); // Azul vivo para destacar
    vec3 colorEmerald = vec3(0.06, 0.73, 0.51); // #10b981 (Cord OK / Accent)

    float t = u_time * 0.4; // Ligeramente más lento para ser más suave
    ...
    // ── Mezcla de Colores ──────────────────────────────────────────────────
    vec3 color = mix(colorNavy, colorBlue, clamp(blob2 * 1.2, 0.0, 1.0));
    color = mix(color, colorBlue, clamp(blob3 * 1.1, 0.0, 1.0));
    color = mix(color, colorEmerald, clamp(blob4 * 1.2, 0.0, 1.0));
    ...
    // Fade más sutil para que se vea bien como banner lleno
