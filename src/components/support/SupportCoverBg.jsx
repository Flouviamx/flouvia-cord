...
// Mismo motor de gradiente que BlogCover.jsx (FBM domain-warp, Reinhard tonemap +
// dither) pero sin overlay de título/watermark — pensado para llenar el hero
// completo de /soporte, no una tarjeta individual.
...
void main() {
  ...
  float glow = exp(-length(uv - vec2(0.5, 0.4)) * 3.5) * 0.06;
  col += u_cc * glow;

  // Fade oscuro hacia arriba (bajo el navbar, mismo patrón que BlueAuroraBg)
  float topFade = smoothstep(0.0, 0.55, uv.y);
  col *= mix(0.45, 1.0, topFade);
  ...
  // Baja la exposición general — el look debe ser oscuro, no pastel
  col *= 0.62;
...
// Paleta más oscura que la de BlogCover (que está pensada para tarjetas chicas):
// navy casi negro → azul marino → azul medio, sin llegar a azure brillante.
const PALETTE = [[0.015, 0.035, 0.085], [0.035, 0.095, 0.22], [0.09, 0.20, 0.46]];
...
export default function SupportCoverBg() {
  ...
  useEffect(() => {
    ...
    gl.uniform3fv(uCa, PALETTE[0]);
    gl.uniform3fv(uCb, PALETTE[1]);
    gl.uniform3fv(uCc, PALETTE[2]);
