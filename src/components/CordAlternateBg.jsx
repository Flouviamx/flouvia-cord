...
const fragmentShader = /* glsl */`
  ...
  void main() {
    ...
    // ── Paleta AI Tech (Estilo ElevenLabs / Modern SaaS) ──
    // Base menos oscura que Obsidian. Un Slate/Charcoal técnico.
    vec3 colorBase    = vec3(0.07, 0.10, 0.15); // #121A26 (Slate 900)
    
    // Tonos "Holográficos" técnicos (Azul claro + Índigo)
    vec3 colorSky     = vec3(0.22, 0.74, 0.97); // #38BDF8 (Sky Blue luminoso)
    vec3 colorIndigo  = vec3(0.40, 0.44, 0.95); // #6670F4 (Tech Indigo)
    
    // Anclaje a la marca Cord
    vec3 colorEmerald = vec3(0.06, 0.73, 0.51); // #10B981 (Cord Emerald)
    ...
    // ── Fusión "AI Tech" ────────────────────────────────────────────────────
    vec3 color = mix(colorBase, colorIndigo, clamp(blob2 * 1.3, 0.0, 1.0));
    color = mix(color, colorSky, clamp(blob3 * 1.1, 0.0, 1.0));
    color = mix(color, colorEmerald, clamp(blob4 * 0.9, 0.0, 1.0)); 
...
export default function CordAlternateBg() {
  ...
  return (
    <div
      ...
      style={{
        ...
        inset:         0,
        zIndex:        0,
        backgroundColor: '#121A26', // Tech Slate 900
        opacity:       visible ? 1 : 0,
