...
export const GET: APIRoute = async ({ request }) => {
    ...
    const [org] = await sql`select nombre from orgs where id = ${orgId}`;
    const rows = await sql`
        select c.id, c.folio, c.total, c.terminos, c.public_token,
               coalesce(c.approved_at, c.created_at) as base, cl.empresa, cl.email
        from cotizaciones c join clientes cl on cl.id = c.cliente_id
        where c.org_id = ${orgId} and c.status in ('approved', 'invoiced') and cl.email is not null and cl.email <> ''`;
    ...
    const candidatos = rows.map((r) => {
        ...
        return { id: r.id as string, folio: r.folio as string, total: num(r.total), token: r.public_token as string, empresa: r.empresa as string, email: r.email as string, vence: due, dias };
    ...
    for (const c of candidatos) {
        ...
        try {
            ...
            if (res.ok) { enviados++; await logAudit(orgId, { accion: 'recordatorio.enviado', entidad: 'cotizacion', entidad_id: c.id, detalle: `${c.folio} → ${c.email}` }); }
