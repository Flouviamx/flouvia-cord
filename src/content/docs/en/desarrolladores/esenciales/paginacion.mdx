---
title: "Pagination"
seoTitle: "Pagination in the Cord API"
...
`GET /api/v1/events` also paginates with a cursor, but its `next_cursor` is an **opaque token**: do not parse or build it, just send it back as-is in `?cursor=`. See [Event history through the API](/en/docs/desarrolladores/herramientas/webhooks#event-history-through-the-api).
