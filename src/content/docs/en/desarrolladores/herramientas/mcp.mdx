...
2. A status card at the top tells you whether you already have an active key and, if an AI
   ...
   **"Create a key for MCP"** button generates a `write` key without leaving the page — it's
   ...
   scope by hand — `write` to use `crear_cotizacion_borrador`, `read` is enough for the rest.)
...
auto-approve without asking the user every time. The 8 read tools are `readOnlyHint: true`;
the two write tools (`crear_cotizacion_borrador` and `crear_factura_borrador`) are
`readOnlyHint: false` and, honestly, `idempotentHint: false` by default — neither one stamps
or sends anything, they only leave a draft ready for a human to review before triggering an
irreversible action (rule 17: nothing that costs real money fires without a human confirming
it). `crear_cotizacion_borrador` is safe to retry if you pass `idempotency_key` (see below);
`crear_factura_borrador` doesn't accept that parameter yet — every call creates a new draft.

## Pagination

`buscar_cliente`, `listar_productos`, and `listar_facturas` return pages, not the whole table:
