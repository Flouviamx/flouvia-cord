...
<Callout type="info">
Each payment (the upfront deposit and the balance) is an independent transaction for fee purposes — Cord Payments charges its fee on each one separately, using the same fee schedule as a normal payment. See [Fees and advantages](/en/docs/pagos/condiciones).
</Callout>
