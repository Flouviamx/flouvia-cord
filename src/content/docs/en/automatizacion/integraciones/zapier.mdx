...
2. **Create a secret key.** In Cord turn on **Developer mode**, open **API** in the Developers panel and create a key with **write** access.
3. **Create a Zap.** Choose Cord as a trigger or an action and paste the key when Zapier asks for it.
</Steps>
...
<Callout type="info">
The API key connection is temporary: we are moving the app to one-click authorization, with no keys to create. When it is ready your Zaps will keep working and you will only need to authorize the account again.
</Callout>
