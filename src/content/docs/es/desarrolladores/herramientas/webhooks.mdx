...
1. Activa el **Modo desarrollador** en Ajustes › Empresa (interruptor al final del índice de Ajustes) — esto abre el **dock de Desarrolladores**, la barra que aparece al fondo de la pantalla en toda la app.
2. Abre la pestaña **Webhooks** del dock.
...
- En un replay manual desde Ajustes ("Reintentar" en el log de entregas) — el payload se reenvía **byte-idéntico**, incluido el `id`.
...
Cuando arregles tu endpoint, desde Ajustes puedes:
...
Puedes rotar el secreto de un endpoint sin perder ni una entrega. Desde Ajustes, botón **"Rotar secreto"**, elige una ventana de solape (1h / 24h / 72h) y confirma — verás el secreto nuevo en claro una sola vez.
...
| `ping` | Solo aparece al usar "Enviar prueba" desde Ajustes — nunca lo dispara un evento real. |
