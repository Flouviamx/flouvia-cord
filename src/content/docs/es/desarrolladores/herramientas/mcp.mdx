...
2. Un tablero de estado arriba te dice si ya tienes una llave activa y, si ya conectaste un
   ...
   si prefieres controlar el scope a mano — `write` para usar `crear_cotizacion_borrador`,
   `read` alcanza para las demás.)
...
auto-aprobar sin preguntarle al usuario cada vez. Las 8 de lectura son `readOnlyHint: true`;
las dos de escritura (`crear_cotizacion_borrador` y `crear_factura_borrador`) son
`readOnlyHint: false` y, honestamente, `idempotentHint: false` por default — ninguna de las
dos timbra ni envía nada, solo dejan un borrador listo para que una persona lo revise antes de
disparar una acción irreversible (regla 17: nada que cueste dinero real se dispara sin que un
humano lo confirme). `crear_cotizacion_borrador` es segura de repetir si le mandas
`idempotency_key` (ver abajo); `crear_factura_borrador` todavía no acepta ese parámetro — cada
llamada crea un borrador nuevo.

## Paginación

`buscar_cliente`, `listar_productos` y `listar_facturas` devuelven páginas, no la tabla completa:
