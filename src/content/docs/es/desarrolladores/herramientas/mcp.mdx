...
2. Genera una Clave de API si no tienes una (necesitas scope `write` para usar `crear_cotizacion_borrador`; `read` alcanza para las demás).
...
peticiones por llave (600/min), y queda en el log de **Ajustes › Desarrolladores › Actividad**
con la tool que se llamó (ej. `/mcp/tools/call:listar_productos`) — así puedes ver exactamente
qué está haciendo tu agente, con qué llave y cuándo.
