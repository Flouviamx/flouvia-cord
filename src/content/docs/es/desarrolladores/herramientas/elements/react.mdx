...
<Callout type="warning">
**`ivaPct` es obligatorio, no un ajuste opcional.** El SDK ya no asume 16% (México) en
silencio si no lo configuras — `useQuoteBuilder` (y por lo tanto `<CordBuilder>`) **lanza un
error** en cuanto se monta si no encuentra una tasa explícita, ni por prop ni por
`<CordProvider>`. Configúrala una sola vez a nivel global —
`<CordProvider ivaPct={0.16}>`— en lugar de repetirla en cada instancia del Builder: así el
total que ve tu cliente nunca diverge del que valida el servidor para tu organización. Un
negocio fuera de México debe pasar SU propia tasa (o `0` si su venta no lleva impuesto) — nunca
asumas 0.16 por default.
</Callout>