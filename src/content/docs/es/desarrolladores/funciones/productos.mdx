...
seoTitle: "API de productos de Cord"
...
La API de listado (`GET /api/v1/productos`) cuenta con un mecanismo de enmascaramiento dinámico (Dynamic Masking) basado en el nivel de confianza de la clave de API con la que te autenticas.
...
Con `sk_live_...` el mismo producto incluye `costo`:

```json
{ "data": [ { "id": "prod_1a2b3c...", "sku": "LIC-PRO-24", "nombre": "Licencia Anual Profesional", "unidad": "licencia", "precio": 12000, "costo": 4000, "activo": true } ] }
```

Con `pk_live_...`, `costo` simplemente no aparece en el objeto (no viene como `null` ni `0` — la clave no existe):

```json
{ "data": [ { "id": "prod_1a2b3c...", "sku": "LIC-PRO-24", "nombre": "Licencia Anual Profesional", "unidad": "licencia", "precio": 12000, "activo": true } ] }
```

Nota: una Publishable Key solo puede usarse en `GET /api/v1/productos` y `POST /api/v1/cotizaciones` — cualquier otra ruta (crear productos, leer clientes, cobranza) responde `403 insufficient_scope`, y además exige un header `Origin`/`Referer` que coincida con los dominios que registraste para esa llave.
