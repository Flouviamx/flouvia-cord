---
title: "Paginación"
seoTitle: "Paginación en la API de Cord"
...
`GET /api/v1/events` también pagina con cursor, pero su `next_cursor` es un **token opaco**: no intentes interpretarlo ni construirlo, solo reenvíalo tal cual en `?cursor=`. Ver [Historial de eventos por API](/docs/desarrolladores/herramientas/webhooks#historial-de-eventos-por-api).
