...
Además, una Publishable Key solo puede usarse en dos rutas: `GET /api/v1/productos` y `POST /api/v1/cotizaciones` — cualquier otro intento responde `403 insufficient_scope`. La petición también debe enviar un header `Origin` o `Referer`; si registraste dominios permitidos para esa llave, Cord verifica que coincidan antes de aceptarla.

<Callout type="info">
**Atención:**
**Nunca expongas tus Claves Secretas.** Si tu clave secreta es comprometida, debes revocarla inmediatamente desde la pestaña **API** del [Cord Workbench](/docs/desarrolladores/herramientas/workbench).
</Callout>
