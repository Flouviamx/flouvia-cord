...
- Al activar la opción "Cobro recurrente mensual", la cotización se convierte en una suscripción real de Cord Payments (ver [Igualas recurrentes](/docs/pagos/igualas)).
- El cliente autorizará su tarjeta de crédito/débito **una sola vez** desde el enlace público al aprobar la propuesta.
- A partir de ese momento, el sistema realizará el cargo total automáticamente cada mes y el dinero caerá directo a tu cuenta bancaria sin fricciones, generando la factura de ese ciclo en automático (CFDI 4.0 en México; el documento que corresponda a tu país en el resto).

### 3. Divisas y Cobertura Cambiaria (FX Lock)

- **Moneda de venta vs. divisa contable:** En B2B es común cotizar en una divisa (Ej. USD o EUR) para protegerse de la devaluación, pero llevar tu contabilidad en la divisa de tu país (Ej. MXN, COP, BRL). Cord soluciona esto de forma nativa cuando ambas difieren.
- **FX Lock (Tasa congelada por 30 días):** Al crear una cotización en una divisa distinta a la contable de tu negocio, Cord obtiene una tasa de cambio real y fechada de un proveedor externo (nunca una tasa inventada: si ninguna fuente responde, la cotización no se crea en esa divisa). Puedes aplicar un porcentaje de *buffer* (cobertura) sobre esa tasa spot para proteger tus márgenes. Una vez enviada la cotización, Cord **congela la tasa aplicada (FX Lock) hasta por 30 días**: si el tipo de cambio se mueve mañana, el total contable se sigue calculando con la tasa que se congeló, protegiendo así la relación comercial y tus ganancias.
