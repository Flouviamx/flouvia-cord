---
title: "Cobrar un anticipo"
description: "Pide un porcentaje por adelantado al aprobar y cobra el saldo según los términos."
category: "Pagos y Depósitos"
order: 4
---

Muchos negocios B2B cobran un porcentaje por adelantado para arrancar un pedido y el resto contra entrega o a crédito. Cord lo maneja de forma nativa: defines un **anticipo** y Cord divide el cobro en dos partes automáticamente.

### Cómo pedir un anticipo

1. Al crear una cotización, en la barra lateral derecha escribe el **% de anticipo** (por ejemplo, 50%).
2. El editor te muestra en vivo cuánto pagará tu cliente al aprobar y cuánto quedará de saldo.
3. Guarda y envía la cotización como siempre.

> [!NOTE]
> Si tu negocio casi siempre pide el mismo anticipo, configúralo como **default** en Ajustes → Cotizaciones. Cada cotización nueva se pre-llenará con ese porcentaje (lo puedes cambiar o quitar por cotización).

### Qué ve tu cliente

En el link público, tu cliente ve el desglose completo desde el inicio: el total, cuánto paga hoy de anticipo y cuánto queda de saldo con su fecha de vencimiento. Al aprobar:

- El **anticipo** es pagable de inmediato (tarjeta, o SPEI si tu negocio cobra en pesos mexicanos), directo a tu banco.
- El **saldo** queda pendiente. Si la cotización es de contado, también es pagable al momento; si es a crédito (Net 30/60), el saldo se puede pagar hasta que llegue la fecha de vencimiento.

Cada parte es un cobro independiente con su propio link de pago. La cotización se marca como **pagada** solo cuando ya no queda ningún cobro pendiente.

### Nota fiscal

El anticipo y el saldo son una herramienta de **cobro**, no de facturación automática. Cord no genera por sí solo el Complemento de Recepción de Pagos (REP) de cada parcialidad. Consulta con tu contador el tratamiento fiscal correcto según si tu operación es un anticipo real o un pago en parcialidades. Ver también [Facturar anticipos](/soporte/facturacion-anticipos).
