...
✅ **Refactor: flujo de "Continuar editando" para usar el editor completo + bugs subsecuentes (jul 2026)** —
   ...
   ⚠️ **Regla a futuro para `PATCH /api/cotizaciones/[id]`:** Siempre definir `actual` antes de cualquier bloque que lo use; usar solo tagged templates de Neon (no `sql.query`); y para actualizar `vigencia`, pasar siempre una `date` calculada desde días, no el número de días directamente.
