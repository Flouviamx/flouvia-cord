...
✅ **Landing de Cord en flouvia.com — acordeón de FAQ + tarjetas con shader físico +
   ...
   • **FAQ convertido a acordeón estilo Cord:** el "Despejando dudas" de `cord.astro`/
     ...
     divergir) — mismo patrón que exige `docs/estado/sistema-de-diseno.md` de este repo para
     `FaqAccordion.astro`.
...
- **Freemium renovable y copy de conversión:** la landing, `/precios`, las FAQs,
  el JSON-LD y las referencias públicas ahora explican Gratis como una cuenta sin
  vencimiento con 5 envíos renovables el día 1 de cada mes (UTC) y hasta 5
  cotizaciones activas. Se añadió `PlanValue.astro` para explicar el recorrido
  natural a Starter (más envíos y sin “Powered by Cord”) y Profesional (equipo,
  seguimiento y cobranza), en español e inglés.
- Se alinearon `pricing.md`, `llms.txt` y el Centro de Ayuda con los precios y la
  disponibilidad reales. El copy dice explícitamente que Scale permite correo
  desde el dominio mediante SMTP, mientras el dominio propio para links aún no
  está disponible. No se publicó un umbral de facturación cobrada porque todavía
  no existe un gate ejecutable para esa regla.
- SEO on-page actualizado: títulos y descripciones apuntan a software de
  cotizaciones online y uso gratuito mensual; el mensaje visible, metadata y
  Product/FAQ JSON-LD comparten la misma oferta. Verificado con `npm run build`,
  `npm run typecheck` y una comprobación responsive en 390 px sin overflow.
