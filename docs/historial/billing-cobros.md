...
las equivalencias falsas entre cancelación y nota de crédito, solicitud y
cancelación confirmada, y retorno del checkout y pago confirmado. El roadmap
incorpora el programa abierto de confiabilidad con propuestas posteriores aún
pendientes. `estado/confiabilidad.md` concentra evidencia, operaciones autorizadas
y criterios de salida. La publicación documental debe partir del commit desplegado
con solo contenido autorizado; no del worktree compartido completo.

---

**Fase 1: cambio de método y registro durable de intentos (7–8 sep 2026)** —
se cerró el primer bloque de confiabilidad del pago público de cotizaciones.

- Verificación y cancelación confirmada antes de sustituir un intento; bloqueos
  ante dinero recibido, autorización o estado incierto. Creación con UUID durable,
  hash de petición y vencimiento conservador para resultados desconocidos.
- Publicación condicional del ID en el cobro antes de confirmar SPEI; historial
  conservado ante carreras. El customer persiste al cambiar a tarjeta.
- Migración aditiva autorizada y aplicada a la base conectada: tabla de intentos,
  índice padre, RLS y FK por negocio/cobro. Preflight: cero cobros y cero intentos
  enlazados; no se ejecutaron cobros, cambios de precios ni reparaciones históricas.
- 35 pruebas nuevas (20 de ruta con proveedor simulado y 15 de SQL/migración en
  PGlite). Se actualizaron contratos desfasados: rutas de revisiones legales,
  dominio propio Profesional ya existente y catálogo de los cinco eventos que
  el buscador de documentación ya emitía. El check de analítica ahora valida el
  wrapper y sus call sites, sin ampliar capturas ni modificar consentimiento.
- Validación final: 567 pruebas en 47 archivos, `test:payments`, tipos y build
  aprobados. Billing LIVE y Billing DB se consultaron sin cambios.
- No se ejecutó despliegue ni pago integrado Stripe TEST. Este bloque no cierra
  toda la fase 1; límites y siguientes controles en el estado de cobros.

---

**Planes EUR aprobados y verificados (6 sep 2026)** — el usuario aprobó los
precios base y, por separado, las tarifas de excedentes.

- Starter/Profesional/Scale: 12/30/70 EUR al mes y 120/300/700 EUR al año.
  IA extra 0,20/0,175/0,15 EUR; factura extra 0,15/0,15/0,10 EUR;
  cada 100 solicitudes API 0,03/0,03/0,02 EUR; usuario extra Pro/Scale 15 EUR/mes.
- Se añadieron y verificaron 17 opciones EUR LIVE sobre los Price existentes
  (6 base y 11 medidos). MXN/USD y suscripciones existentes sin cambios.
  Developer sigue por ventas, sin inventar una tarifa EUR.
- Nuevas organizaciones ES/DE/FR usan EUR; el contrato existente conserva su
  moneda, incluida la de un customer con primera factura todavía incompleta.
  Checkout comprueba la moneda del resumen antes de crear una suscripción cobrable.
- Landing, precios, checkout, Plan y paywall reconocen EUR. La comparativa
  conserva los tres decimales de excedentes, el selector manual prevalece sobre
  una respuesta tardía de geolocalización y las tarjetas muestran el total anual.
  El enlace de registro preserva el ciclo anual; Gratis conserva su CTA de alta.
- 27 pruebas nuevas: tarifas, catálogo, reejecución sin cambios, rechazo de drift,
  conservación de moneda y creación de suscripción simulada. Total 436 pruebas;
  `test:payments`, tipos, build, CSS, Billing LIVE y Billing DB pasan. Navegador
  en ES/EN y móvil, sin completar un pago. No se emitieron cobros ni se publicó
  el worktree; los cambios de interfaz requieren despliegue.

**Cierre de pendientes operativos y solicitud EUR (6 sep 2026)** — el usuario
autorizó ejecutar los pendientes de la tanda anterior.

- Se aplicó la migración de conciliación sobre la base conectada: tres columnas,
  ledger de reembolsos y políticas ENABLE/FORCE RLS; sin cambios en los tres
  documentos existentes. Se añadió runner acotado con validación, transacción,
  límite de espera de locks, modo lectura por defecto y prueba PostgreSQL local.
- USD no faltaba solamente en Starter: faltaba en los 23 Price base/medidos.
  Se añadieron las tarifas ya declaradas por Cord a todos ellos, sin cambiar
  MXN ni modificar suscripciones. La auditoría LIVE ya pasa.
- Los scripts de lectura ahora expanden `currency_options`, como exige Stripe;
  el script de alta detecta las opciones existentes al reejecutarse.
- Se corrige el copy de pago de Build y las referencias operativas de ayuda.
  La identificación de Stripe en retención de datos conserva su explicación
  de privacidad, con una excepción puntual en el linter de marca.
- Suite de 409 pruebas y comando completo `test:payments` correctos. EUR queda
  pendiente de definir importes, con propuesta enviada de 12/30/70 EUR al mes
  para Starter/Profesional/Scale y anualidad de diez mensualidades. No se activó
  una tarifa EUR ni se desplegó el worktree.

**Recurrencias y link público (6 sep 2026)** — ambas mejoras autorizadas,
implementadas en ese orden y verificadas localmente.

- Recurrencias: reclamación condicional de cada periodo, respeto de pausas y
  ediciones, precisión del timestamp, fechas válidas, cliente de la misma org,
  reserva de timbrado y aviso de correo no enviado. Se mantiene la política
  existente de avanzar fecha antes de emitir; no se añadió reintento automático.
- Link: siguiente paso, contacto de la empresa, actualización de saldo y espera
  de confirmación basada en el ledger, sin dar por recibido un pago por su URL.
  Respuesta pública mínima, sin caché/referrer; notas y cancelaciones con acciones
  acordes a su estado. Mejor acceso a los importes en móvil y errores visibles.
- 38 pruebas nuevas (19 por bloque), incluidas consultas PostgreSQL locales para
  reclamación de periodo y pertenencia del pago. Total de la suite: 408 pruebas.
  Tipos y build correctos; revisión del navegador con datos ficticios.
- Persisten los pendientes anteriores de white-label y Price Starter mensual
  sin USD. La migración de conciliación sigue sin aplicar: estos cambios no son
  un despliegue ni acreditan funcionamiento completo en producción.

**Conciliación de Invoicing (6 sep 2026)** — continuación local autorizada de
la opción recomendada; sin migrar bases externas, desplegar ni mover dinero.

- El saldo integra notas emitidas, cobros brutos y reembolsos efectivos; el
  exceso neto se muestra como importe por devolver en el detalle y link público.
- Borradores de notas reservan capacidad bajo lock del original. Emisión y
  cancelación actualizan nota y saldo en una sola transacción local.
- Se añade ledger tenant de reembolsos con FORCE RLS; el webhook verifica el
  estado vigente y recupera devoluciones recibidas antes que el pago.
- Pagos manuales se acotan al saldo actual; los eventos externos fallidos no
  se confirman como aplicados. Los locks por PI/documento y la idempotencia
  evitan repetir movimientos al reintentar.
- Se añade PGlite como dependencia de desarrollo para ejecutar SQL PostgreSQL
  y políticas RLS sin una base externa. Dieciséis pruebas locales cubren
  aritmética, reservas, reintentos, aislamiento y rollback de emisión; otras
  tres pruebas cubren el webhook firmado. Total: 370 pruebas.
- La migración queda preparada para revisión. Despliegue, revisión de datos
  históricos y ejecución de reembolsos siguen requiriendo autorización propia.

**Tanda fiscal de la auditoría (5 sep 2026)** — autorizada para código y pruebas
locales, sin timbrar/cancelar comprobantes reales ni desplegar.

- MX transmite impuestos y retenciones del snapshot en lugar de depender del
  IVA predeterminado del proveedor; valida base, tipo e importes antes de enviar.
- Las notas conservan tasas mixtas y retenciones, emiten egreso relacionado con
  el UUID original y no admiten cobros. Se protege el borrador contra el editor
  de ingresos y se habilita su emisión desde el detalle.
- Las cancelaciones distinguen aceptación, espera, rechazo, expiración e
  incertidumbre; la consulta explícita y la consulta previa al DELETE recuperan
  respuestas perdidas. Solo la aceptación confirmada publica `invoice.voided`.
- 46 pruebas nuevas (351 totales) con fetch y DB simulados. Tipos, build y
  controles fiscales/tenancy pasan; persisten los fallos previos de whitelabel
  y del Price Starter mensual sin USD. La aplicación contable del crédito,
  sustituciones y sincronización fiscal automática quedan fuera de esta tanda.

**Primera tanda de correcciones de la auditoría (5 sep 2026)** — autorizada para
código y pruebas locales; sin despliegue ni cambios de configuración externa.

- Las APIs de sesión aplican la exigencia de 2FA, con recuperación acotada y
  verificación que falla cerrada. Se conserva el comportamiento previo de los
  gates de navegación que no forman parte de esta tanda.
- Invoice admite abonos sucesivos del mismo importe, espera el asiento del
  anterior y evita crear otro intento por errores de consulta o cambio de monto
  mientras un pago sigue en proceso. La creación usa idempotencia por predecesor
  y publicación condicional del id con el mismo saldo.
- Se elimina el case duplicado de `refund.updated`, preservando Build y Payments.
- Las descargas públicas PDF/XML validan el token y la organización del documento,
  con respuesta privada sin caché y no-referrer; el endpoint interno mantiene sesión.
- Se agregan pruebas de rutas, middleware y webhook firmado con proveedores/DB
  simulados. Permanecen pendientes los demás hallazgos de la auditoría: RLS real,
  configuración USD, fiscal, recurrencias y conciliación integral, entre otros.
...
✅ **Auditoría completa de `docs.cordhq.app` para Cord Pagos (ago 2026)** — André pidió
   ...
   • **Editadas:** `anticipos.mdx`/`igualas.mdx` (cross-link a la tarifa real por cobro).
     ...
     "Consolidación de Informes" (ver `app-features.md`) ese pronóstico vive en
...
### 2026-09-08 — fase 1: comisiones de factura directa

Se cubre el camino `metadata.documento_id`: antes aplicaba saldo y omitía
`comisiones`. El nuevo conciliador usa `documento_pagos` como relación org/factura,
la clave existente org/PI y el cargo de la cuenta conectada. Guarda base/impuesto/total
al preparar el intento, valida captura/divisa/importes y conserva la fecha del cargo.
No estima costos sin balance ni reconstruye IVA histórico. `needs_review` queda fuera
del borrador mensual; `pending` espera datos verificables. Los eventos tardíos de
creación de comisión no revierten una devolución ni eliminan revisión.

Pruebas de ruta, webhook firmado y SQL cubren duplicados, fallo/reintento, aislamiento,
FX, desglose ausente y actualización posterior del balance. No hubo migración, cobro,
timbrado ni backfill nuevo. Push y publicación se posponen por instrucción del usuario.
También se corrigió la legibilidad de tablas de documentación en escritorio/móvil.

El mismo bloque prepara CI de la aplicación en `cord-reliability.yml`: pruebas y
contratos de pagos, tipos, build y CSS con permisos de lectura y sin secretos LIVE.
La excepción en `.gitignore` permite versionar solo ese workflow nuevo. Todavía no
se activó en GitHub ni se impuso como requisito para merge/despliegue. Se verificó
la suite y compilación en copia sin credenciales, reutilizando dependencias locales.

### 2026-09-08 — fase 1: orden de renovación de sesiones

Se encontró que `validateSession` renovaba `last_used_at` antes del gate y que la
API no comprobaba inactividad. Ahora la lectura de identidad es pasiva y el
acceso protegido comprueba/actualiza la sesión bajo lock con hora de Postgres.
Una sesión vencida se elimina sin renovarse; API devuelve 401 y páginas vuelven
a login, con handoff al apex para Billing. Recuperación de 2FA permanece acotada
y no renueva actividad. Cada request protegido cuenta como actividad; el TTL y
la cookie se renuevan con throttle independiente. No hubo migración ni cambio
en la política configurada por las organizaciones. Pruebas SQL y middleware
cubren concurrencia, rutas públicas, recuperación, TTL, revocación y suspensión.

### 2026-09-08 — unificación del checkout aprobada

El usuario eligió usar el flujo actual. La API `POST /api/q/[token]/checkout`
conserva `{ url }`, valida elegibilidad y devuelve `/q/<token>/pay`; ya no crea
Customers ni Checkout Sessions. El flag anterior y las tarifas aceptadas no
cambian. Se mantiene el procesamiento de eventos de sesiones ya emitidas.

Auditoría de solo lectura (03:06 UTC del 9-sep / noche del 8-sep en México):
tres cuentas conectadas LIVE, una org con `checkout_v2 = false`, cero sesiones
`open` o `complete` y ninguna lectura incompleta. Es evidencia de las cuentas
en la base configurada, no de equivalencia entre esa base y todo despliegue.
